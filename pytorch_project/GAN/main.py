...
class Config1(object):
    data_path = '/home/zhangp/Documents/data/faces/'
    num_workers = 4
    image_size = 96
    batch_size = 256
    max_epochs = 200
...
class Config(object):
    data_path = '/home/zhangp/Documents/data/faces/'  # 数据集存放路径
...
def generate(**kwargs):
    ...
    map_location = lambda stronge, loc: storage
    netd.load_state_dict(t.load(opt.netd_path, map_location = map_location))
    netg.load_state_dict(t.load(opt.netg_path, map_location = map_location))
    netd.to(device)
    netg.to(device)
